...
class Agent(BaseModel):
    ...
    async def from_db_dict(
        ...
        memories_data = await database.get_by_field(Tables.Memories, "agent_id", str(agent_dict["id"]))
    ...
    async def from_id(cls, id: UUID, context: WorldContext):
        ...
        locations_data = await database.get_by_field(Tables.Locations, "world_id", agent["world_id"])
    ...
    async def _get_memories(self):
        data = await (await get_database()).get_by_field(Tables.Memories, "agent_id", str(self.id))
