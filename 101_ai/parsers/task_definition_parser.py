...
class BackendEndpointDefinition(BaseModel):
    ...
    pseudocode: str = Field(
        description="The pseudocode for this endpoint in plain english"
    )
...
class CronJobDefinition(BaseModel):
    ...
    url: str = Field(
        description="The backend endpoint url to trigger e.g. /endpoint-url"
    )


class TaskDefinition(BaseModel):
    backend_endpoints: list[BackendEndpointDefinition] = Field(
        description="A list of endpoints for the backend web-server that will be triggered by cron tasks or frontend interactions"
    )
    cron_jobs: list[CronJobDefinition] = Field(
        description="A list of necessary cron jobs and which functions they call"
    )
    env_variables: list[str] = Field(
        description="A list of all necessary environment variables."
    )
