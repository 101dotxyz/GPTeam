...
class SubabaseDatabase(DatabaseProviderSingleton):
    client: Client
    ...
    async def get_by_id(self, table: Tables, id: str) -> List[Dict[str, Any]]:
        return (await self.client.table(table.value).select("*").eq("id", id).execute()).data

    async def get_by_ids(self, table: Tables, ids: list[str]) -> List[Dict[str, Any]]:
        return (await self.client.table(table.value).select("*").in_("id", ids).execute()).data
    ...
    async def get_by_field(self, table: Tables, field: str, value: Any, limit: int = None) -> List[Dict[str, Any]]:
        if limit is not None:
            return (await self.client.table(table.value).select("*").eq(field, value).limit(limit).execute()).data
        return (await self.client.table(table.value).select("*").eq(field, value).execute()).data

    async def get_by_field_contains(self, table: Tables, field: str, value: Any, limit: int = None) -> List[Dict[str, Any]]:
        if limit is not None:
            return (await self.client.table(table.value).select("*").contains(field, value).limit(limit).execute()).data
    ...
    async def get_recent_events(self, world_id: str, limit: int) -> List[Dict[str, Any]]:
        return (await self.client.table("Events")
                .select("*, location_id(*)")
    ...
    async def create(cls):
        ...
        try:
            ...
            print_to_console(f"Supabase Error", LogColor.ERROR, "Either there was an issue with your database connection or the tables do not exist. Please check your database connection and try again.")
            print(e)
            exit(1)
