...
class PromptString(Enum):
    ...
    REACT = "You are a role-playing AI, playing the role of {full_name}.\n\nGiven the following information about your character and context, decide whether they should continue with their current plans (\"maintain_plans\"), or if they should make new plans based on the new events (\"replan\"). If the events have a material impact on the character's goals or existing plans, they should replan. If there are plans that are conflicting, redundant or no longer needed, you should replan to address those issues. Always include a 'thought process' that describes why the reaction was chosen and what should be done next.\n\n{format_instructions}\n\nHere's some information about your character:\n\nName: {full_name}\n\nBio: {private_bio}\n\nGoals: {directives}\n\nHere's some context about your character at this moment:\n\nLocation Context: {location_context}\n\nRecent Activity: {recent_activity}\n\nConversation History: {conversation_history}\n\nHere is your characters current plans: {current_plans}\n\nHere are the new events that have occured sincce your character made these plans: {event_descriptions}.\n"
