...
class PromptString(Enum):
    ...
    RECENT_ACTIIVITY = (
        "Given the following memories, generate a few sentences summarizing what {name} has been doing lately.\n\n{memory_descriptions}"
    )

    MAKE_PLANS = (
        "You are a plan generating AI. Given the character's bio, goals, recent activity, and current plans, generate additional plans for them to carry out, such that the final set of plans include at least {time_window} of activity. \n\nName: {name}\nBio: {bio}\nGoals: {directives}\nRecent Activity: {recent_activity}\nCurrent Plans: {current_plans}\n\n{format_instructions}"
    )
