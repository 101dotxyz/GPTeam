...
def discord_listener():
    ...
    bot = hikari.GatewayBot(token=ANNOUNCER_DISCORD_TOKEN, intents=(hikari.Intents.ALL_UNPRIVILEGED | hikari.Intents.MESSAGE_CONTENT))
    ...
    async def inbound_message(event: hikari.GuildMessageCreateEvent) -> None:
        ...
        if me.id in event.message.user_mentions_ids:
            message = ChatCompletion.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are an AI that impersonates characters from fiction"},
                    {"role": "user", "content": "Can you send me something ominous that Big Brother from 1984 might say? It doesn't have to be literally from the book, just more in the spirit of the character. Reply with only the message content, no quotes, or other commentary."}
                ],
                temperature=0.9,
        ...
        locations = await database.get_by_field(Tables.Locations, "channel_id", str(event.message.channel_id))
        if len(locations) == 0:
            return
        ...
        if event.message.type == hikari.MessageType.REPLY:
            ...
            if referenced_message is None:
                return
            referenced_event = await database.get_messages_by_discord_id(str(referenced_message.id))
            if len(referenced_event) == 0:
                return
            referenced_event = referenced_event[0]
            question = referenced_message.content.split(":")[1].strip()
            await database.insert(Tables.Messages, {
